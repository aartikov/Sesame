...
class PagedLoadingTest {
    ...
    fun `shows previous data during refresh`() = runBlockingTest {
        ...
        assertEquals(
            State.Data(
                1,
                listOf("Previous value1", "Previous value2"),
    ...
    fun `replaces previous data after refresh`() = runBlockingTest {
        ...
        val loading = PagedLoading(
            ...
            initialState = State.Data(
                2,
                listOf("Previous value1", "Previous value2", "Previous value3")
            )
        )
    ...
    fun `shows loading more progress`() = runBlockingTest {
        ...
        assertEquals(
            State.Data(1, listOf("Value1", "Value2"), DataStatus.LoadingMore),
            loading.state
        )
        cancelJobs()
    }
    ...
    fun `shows error when loading more failed`() = runBlockingTest {
        ...
        assertEquals(State.Data(1, listOf("Value1", "Value2")), loading.state)
        val expectedStateDuringLoading =
            State.Data(1, listOf("Value1", "Value2"), status = DataStatus.LoadingMore)
        assertEquals(
            listOf(Event.Error(LoadingFailedException(), expectedStateDuringLoading)),
            events
        )
        cancelJobs()
    }
    ...
    fun `stop paging when end of data is reached`() = runBlockingTest {
        ...
        val loading = PagedLoading(
            this,
            loader,
            initialState = State.Data(2, listOf("Value1", "Value2", "Value3"))
        )
        ...
        assertEquals(
            State.Data(2, listOf("Value1", "Value2", "Value3"), DataStatus.FullData),
            loading.state
        )
        cancelJobs()
    }
    ...
    fun `loads data without duplicates with DuplicateRemovingDataMerger`() = runBlockingTest {
        ...
        val loading = PagedLoading(
            this,
            loader,
            initialState = State.Data(0, listOf("Value1", "Value2", "Value3")),
        ...
        assertEquals(
            State.Data(1, listOf("Value1", "Value2", "Value3", "Value4", "Value5")),
            loading.state
        )
        cancelJobs()
    }
